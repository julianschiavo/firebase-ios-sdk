...
class Document {
 ...
 public:
   ...
   */
  static Document InvalidDocument(const DocumentKey& document_key);

  /**
   * Creates a new document that is known to exist with the given data at the
   * given version.
   */
  static Document FoundDocument(const DocumentKey& document_key,
                                const SnapshotVersion& version,
                                ObjectValue value);

  /** Creates a new document that is known to not exisr at the given version. */
  static Document NoDocument(const DocumentKey& document_key,
                             const SnapshotVersion& version);
   ...
   * base document).
   */
  static Document UnknownDocument(const DocumentKey& document_key,
                                  const SnapshotVersion& version);
   ...
   * version.
   */
  Document& ConvertToNoDocument(const SnapshotVersion& version);
   ...
   * known base document).
   */
  Document& ConvertToUnknownDocument(const SnapshotVersion& version);
