...
void LocalStore::ApplyBatchResult(const MutationBatchResult& batch_result) {
  ...
  DocumentMap LocalStore::RejectBatch(BatchId batch_id) {
    return persistence_->Run("Reject batch", [&] {
      ...
      HARD_ASSERT(to_reject.has_value(),
  ...
  model::DocumentMap LocalStore::ApplyRemoteEvent(
    ...
    return persistence_->Run("Apply remote event", [&] {
      ...
      for (const auto& entry : remote_event.target_changes()) {
        ...
        target_cache_->RemoveMatchingKeys(change.removed_documents(),
                                          target_id);
        ...
        if (!resume_token.empty()) {
          ...
          if (ShouldPersistTargetData(new_target_data, old_target_data,
                                      change)) {
  ...
  TargetData LocalStore::AllocateTarget(Target target) {
    ...
    if (target_data_by_target_.find(target_id) ==
        target_data_by_target_.end()) {
  ...
  LruResults LocalStore::CollectGarbage(LruGarbageCollector *
                                        garbage_collector) {
  ...
  absl::optional<bundle::NamedQuery> LocalStore::GetNamedQuery(
    ...
    return persistence_->Run(
        "Get named query", [&] { return bundle_cache_->GetNamedQuery(query); });
  }
  ...
  DocumentMap LocalStore::PopulateDocumentChanges(
    ...
    OptionalDocumentMap changed_docs;
    ...
    OptionalDocumentMap existing_docs =
    ...
    for (const auto& kv : documents) {
      ...
      const SnapshotVersion& read_time =
          search_version != document_versions.end() ? search_version->second
                                                    : global_version;
